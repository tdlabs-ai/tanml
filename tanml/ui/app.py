...
# --- NARRATIVE HELPERS (Smart Heuristics) ---
def _story_performance(metrics, task_type):
    """Generates a dynamic sentence about model performance."""
    s = []
    if task_type == "classification":
        auc = metrics.get("roc_auc", 0)
        if auc:
            s.append(f"The model achieved an ROC AUC of **{auc:.3f}**.")
            if auc > 0.9: s.append("This indicates **excellent** discriminatory power.")
            elif auc > 0.75: s.append("This performance is considered **good**.")
            elif auc > 0.6: s.append("The model has **moderate** predictive skill.")
            else: s.append("The performance is relatively weak, suggesting more features or data may be needed.")
        ...
        if r2 > 0.8: s.append("This is a **high-precision** model.")
        elif r2 < 0.3: s.append("The low R2 suggests the features explain very little of the target's behavior.")
        
    return " ".join(s)

def _story_features(metrics_df, top_n=3):
    """Analyzes feature dominance."""
    ...
    total_power = metrics_df["Power"].sum()
    if total_power <= 0: return ""
    
    top_power = top["Power"].sum()
    share = top_power / total_power
    
    s = f"The top {top_n} features (**{', '.join(top_names)}**) drive **{share:.0%}** of the total predictive signal."
    
    if share > 0.8:
        s += " The model is heavily reliant on these few features (High Concentration)."
    elif share < 0.3:
        s += " The predictive power is well-distributed across many features."
        
    return s

def _story_overfitting(train_m, test_m):
    """Checks for train-test divergence."""
    s = []
    # Pick a key metric (Prioritize standard ones)
    ...
    if metric:
        ...
        problematic = False
        if is_error:
             # Train Error < Test Error -> Delta < 0
             if delta < -0.1: problematic = True
        else:
             # Train Score > Test Score -> Delta > 0
             if delta > 0.1: problematic = True
             
        if problematic:
            s.append("⚠️ **Significant Overfitting Detected**: The model performs much better on training data than unseen test data. Consider regularization or reducing complexity.")
        elif abs(delta) > 0.05:
            s.append("There is **mild divergence** between Train and Test, but it may be within acceptable limits.")
        else:
            s.append("The model generalizes well, with **consistent performance** across both datasets.")
            
    else:
        # Fallback / Debug
        tr_keys = list(train_m.keys()) if train_m else "None"
        s.append(f"Could not analyze stability. metrics not matched in both datasets. Keys found: {str(tr_keys)[:50]}...")
            
    return " ".join(s)

def _story_drift(drift_data):
    """Summarizes potential data drift."""
    if not drift_data: return "No drift analysis performed."
    
    # Count drifting features
    high_drift = [d for d in drift_data if d['PSI'] > 0.2]
    med_drift = [d for d in drift_data if 0.1 < d['PSI'] <= 0.2]
    
    if len(high_drift) > 0:
        feats = ", ".join([d['Feature'] for d in high_drift[:3]])
...
def _story_stress(stress_data):
    """Analyzes robustness under noise."""
    if not stress_data: return ""
    
    # Typically one row for the main test
    row = stress_data[0]
    s = []
    
    # Check for Classification (Accuracy/AUC)
    if "delta_accuracy" in row:
        acc = row.get("accuracy", 0)
        delta = row.get("delta_accuracy", 0)
        s.append(f"After perturbing 20% of the test data, Accuracy shifted from baseline to **{acc:.3f}** (Delta: **{delta:.3f}**).")
        
        if delta < -0.10:
             s.append("⚠️ **High Sensitivity**: The model's performance drops significantly with noise. It may be brittle.")
        elif delta < -0.02:
             s.append("The model shows **moderate sensitivity** to noise but remains usable.")
        else:
             s.append("✅ **Robust**: The model maintains performance even with noisy data.")
             
    # Check for Regression (RMSE/R2)
    elif "delta_rmse" in row:
        rmse = row.get("rmse", 0)
        d_rmse = row.get("delta_rmse", 0)
        # RMSE Increase = Bad. Delta > 0.
        s.append(f"Under stress, RMSE changed by **{d_rmse:.3f}** (Stressed RMSE: {rmse:.3f}).")
        
        if d_rmse > (rmse * 0.2): # Rough heuristic: >20% increase
             s.append("⚠️ **High Sensitivity**: Error rates spike significantly with noisy inputs.")
        else:
             s.append("✅ **Robust**: Error rates remain stable despite noise.")
             
    return " ".join(s)

def _story_shap(shap_res):
    """Summarizes top drivers from SHAP."""
    if not shap_res or "top_features" not in shap_res: return ""
    
    top = shap_res["top_features"]
    if not top: return ""
    
    # Check keys
    first = top[0]
    k_feat = "feature" if "feature" in first else "Feature"
    
    names = [str(d.get(k_feat, "Unk")) for d in top[:3]]
    s = f"According to SHAP analysis, the top 3 drivers of model predictions are: **{', '.join(names)}**."
    s += " These features have the highest average impact on the output."
...
# ---------------------------------------------------------------------
# CV Helper: Runs Repeated K-Fold and computes stats for report
# ---------------------------------------------------------------------
    ...
    for train_idx, test_idx in cv.split(X_arr, y_enc):
        ...
        if task_type == "classification":
            ...
            if y_prob is not None:
                try: _add("roc_auc", roc_auc_score(y_val, y_prob))
                except: pass
                try: _add("pr_auc", average_precision_score(y_val, y_prob))
                except: pass
                try: _add("brier", brier_score_loss(y_val, y_prob))
                except: pass
                try: _add("log_loss", log_loss(y_val, y_prob))
                except: pass
                try: _add("gini", 2 * roc_auc_score(y_val, y_prob) - 1)
                except: pass
                try:
                    p0 = y_prob[y_val==0]
                    p1 = y_prob[y_val==1]
                ...
                if "curves" not in scores: scores["curves"] = {"roc": [], "pr": []}
                try:
                ...
                try:
                    ...
                    scores["curves"]["pr"].append((rec, prec, th_pr)) # Note: recall on x, precision on y typically
                except: pass

                # --- Pooled OOF Data for Lift/Gain/Confusion ---
                if "oof" not in scores: scores["oof"] = {"y_true": [], "y_prob": [], "y_pred": []}
                ...
                if "y_probs" not in scores: scores["y_probs"] = []
                if "y_trues" not in scores: scores["y_trues"] = []
                scores["y_probs"].append(y_prob)
                scores["y_trues"].append(y_val)

            try: _add("f1", f1_score(y_val, y_pred))
            except: pass
            try: _add("precision", precision_score(y_val, y_pred))
            except: pass
            try: _add("recall", recall_score(y_val, y_pred))
            except: pass
            try: _add("accuracy", accuracy_score(y_val, y_pred))
            except: pass
            try: _add("bal_acc", balanced_accuracy_score(y_val, y_pred))
            except: pass
            try: _add("mcc", matthews_corrcoef(y_val, y_pred))
            except: pass
            
        else:
            try: _add("rmse", np.sqrt(mean_squared_error(y_val, y_pred)))
            except: pass
            try: _add("mae", mean_absolute_error(y_val, y_pred))
            except: pass
            try: _add("median_ae", median_absolute_error(y_val, y_pred))
            except: pass
            try: _add("r2", r2_score(y_val, y_pred))
            except: pass
            
            # --- Pooled OOF Data for Predicted vs Actual ---
            if "oof" not in scores: scores["oof"] = {"y_true": [], "y_pred": []}
            scores["oof"]["y_true"].extend(y_val.tolist())
            scores["oof"]["y_pred"].extend(y_pred.tolist())
