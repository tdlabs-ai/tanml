...
def render_model_development_page(run_dir):
    ...
    with st.expander("⚙️ CV Settings", expanded=True):
        ...
        if task_type == "classification":
            st.info(
                f"📊 **Method**: Repeated Stratified K-Fold ({n_folds} folds × {n_repeats} repeats = {n_folds * n_repeats} total fits)\n\n*Stratified* ensures each fold maintains the same class distribution as the full dataset."
            )
        else:
            st.info(
                f"📊 **Method**: Repeated K-Fold ({n_folds} folds × {n_repeats} repeats = {n_folds * n_repeats} total fits)\n\n*Standard K-Fold* for regression - randomly splits data into K equal parts."
            )
    ...
    if st.button("Run Development Experiments", type="primary"):
        try:
            ...
            if len(temp_df) == 0:
                st.error(
                    "No valid data rows remaining. Check if your Target column contains all NaNs or Infs."
                )
                st.stop()
            ...
            if "could not convert string to float" in msg:
                st.info(
                    "💡 **Tip**: It looks like some features contain text or objects. Models need numbers! Try using **Data Preprocessing** to encode categorical variables."
                )
            elif "contains NaN" in msg or "NaN" in msg or "infinity" in msg:
                st.info(
                    "💡 **Tip**: Your features contain missing values (NaN) or infinite values. Try using **Data Preprocessing** to impute missing values."
                )
    ...
    if "dev_results" in st.session_state:
        ...
        with tab_met:
            res_data = []
            for m, v in stats.items():
                if m in ["oof", "curves", "threshold_info", "y_probs", "y_trues"]:
                    continue
        ...
        with tab_plot:
                ...
                if task_type == "classification":
                    ...
                    with c_d4:
                        ...
                        if "y_probs" in stats and "y_trues" in stats:
                            ...
                            for y_true_f, y_prob_f in zip(
                                stats["y_trues"], stats["y_probs"], strict=False
                            ):
                                try:
                            ...
                            if n0 > 0:
                                ax_ks.plot(
                                    y0, np.arange(1, n0 + 1) / n0, "r", label="Neg CDF", lw=2
                                )
                            if n1 > 0:
                                ax_ks.plot(
                                    y1, np.arange(1, n1 + 1) / n1, "b", label="Pos CDF", lw=2
                                )

                            # KS line
                            x_base = np.linspace(0, 1, 1000)
                            c0 = (
                                np.interp(x_base, y0, np.arange(1, n0 + 1) / n0, left=0, right=1)
                                if n0 > 0
                                else np.zeros(1000)
                            )
                            c1 = (
                                np.interp(x_base, y1, np.arange(1, n1 + 1) / n1, left=0, right=1)
                                if n1 > 0
                                else np.zeros(1000)
                            )
                            d = np.abs(c0 - c1)
                            ...
                            ax_ks.plot(
                                [ks_x, ks_x],
                                [c1[np.argmax(d)], c0[np.argmax(d)]],
                                "k--",
                                lw=2,
                                label=f"KS={np.max(d):.3f}",
                            )
                    ...
                    r2_val = 1 - (
                        np.sum(residuals**2) / np.sum((y_true_oof - np.mean(y_true_oof)) ** 2)
                    )
                    ...
                    with c_r1:
                        ...
                        # Perfect fit line
                        lims = [
                            min(y_true_oof.min(), y_pred_oof.min()),
                        ...
                        ax_pva.set_title(
                            f"Predicted vs Actual (R² = {r2_val:.4f})",
                            fontsize=12,
                            fontweight="bold",
                        )
                        ax_pva.legend()
                    ...
                    with c_r4:
                        ...
                        ax_qq.set_title(
                            "Q-Q Plot (Normality Check)", fontsize=12, fontweight="bold"
                        )
                    ...
                    if (
                        "rmse" in stats
                        and isinstance(stats["rmse"], dict)
                        and "raw" in stats["rmse"]
                    ):
                        st.write("**CV Metric Distribution**")
                        c_r5, _c_r6 = st.columns(2)
                        ...
                        with c_r5:
                            ...
                            for metric_key in ["rmse", "mae", "r2"]:
                                if (
                                    metric_key in stats
                                    and isinstance(stats[metric_key], dict)
                                    and "raw" in stats[metric_key]
                                ):
                            ...
                            if metric_data:
                                ...
                                ax_box.set_title(
                                    "CV Metric Distribution (across folds)",
                                    fontsize=12,
                                    fontweight="bold",
                                )
            ...
            if "development" not in st.session_state["report_buffer"]:
                st.session_state["report_buffer"]["development"] = {}
        ...
        with t_f_p:
            ...
            if task_type == "classification":
                ...
                with c_d2:
                    if hasattr(model, "feature_importances_") or hasattr(model, "coef_"):
                        ...
                        if fi is not None:
                            ...
                            df_imp = (
                                pd.DataFrame({"Feature": features, "Imp": fi})
                ...
                if y_prob is not None:
                    with st.expander("Diagnostic Curves (Full Model)", expanded=True):
                        ...
                        c_r1, c_r2 = st.columns(2)
                        with c_r1:
                            fpr, tpr, _th_roc = roc_curve(y, y_prob)
                            auc_roc = roc_auc_score(y, y_prob)
                            fig_roc, ax_roc = plt.subplots(figsize=(5, 5))
                            ax_roc.plot(
                                fpr, tpr, color="blue", lw=2, label=f"ROC (AUC = {auc_roc:.3f})"
                            )
                        ...
                        with c_r2:
                            ...
                            ax_pr.plot(
                                rec, prec, color="green", lw=2, label=f"PR (AUC = {auc_pr:.3f})"
                            )
                            ax_pr.set(
                        ...
                        with c_r3:
                            ...
                            ax_f1.plot(th_pr, f1_plot, color="purple", lw=2, label="F1 Score")
                            ax_f1.axvline(
                                x=best_thresh,
                                color="red",
                                linestyle="--",
                            ...
                            ax_f1.set(
                                title="F1 vs Threshold", xlabel="Threshold", ylabel="F1 Score"
                            )
                    ...
                    if hasattr(model, "feature_importances_") or hasattr(model, "coef_"):
                        ...
                        with c_d5:
                            st.write("**Feature Importance**")
                                ...
                                model, "feature_importances_", getattr(model, "coef_", None)
                            )
                            if fi is not None:
                                if len(fi.shape) > 1:
                                    fi = fi[0]
                                df_imp = (
                                    pd.DataFrame({"Feature": features, "Importance": np.abs(fi)})
            ...
            # Save to buffer
            if (
                "report_buffer" in st.session_state
                and "development" in st.session_state["report_buffer"]
            ):
