...
def render(context):
    ...
    def _plot_diagnostics(col, title, y_true, y_pred, X_in, img_dict, prefix):
        ...
        if context.task_type == "classification":
            ...
            if hasattr(context.model, "predict_proba"):
                try:
                    ...
                    with col.expander("Diagnostic Curves", expanded=True):
                        ...
                        # 1. ROC
                        fpr, tpr, _th_roc = roc_curve(y_true, y_prob)
            ...
            r2_val = (
                1 - (np.sum(residuals**2) / np.sum((y_true - np.mean(y_true)) ** 2))
                if len(y_true) > 1
                else 0
            )
            ...
            with col.expander("Regression Diagnostics", expanded=True):
                ...
                with sub_c1:
                    ...
                    lims = [
                        min(y_true_s.min(), y_pred_s.min()),
                        max(y_true_s.max(), y_pred_s.max()),
                    ]
                    ax_pva.plot(lims, lims, "r--", lw=2)
                    ax_pva.set_xlabel("Actual", fontsize=9)
                    ax_pva.set_ylabel("Predicted", fontsize=9)
                    ax_pva.set_title(
                        f"Pred vs Actual (R²={r2_val:.3f})", fontsize=10, fontweight="bold"
                    )
