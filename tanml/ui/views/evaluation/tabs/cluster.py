...
def render(context):
    ...
    st.caption(
        "Evaluates how well the **Testing Data** covers the input space defined by the **Training Data** clusters. Low coverage may indicate the model is being applied to out-of-distribution (OOD) samples."
    )
    ...
    with col_clust_btn:
        run_cluster = st.button(
            "🎯 Run Cluster Coverage Check", type="secondary", key="btn_cluster_cov"
        )
        ...
        with st.spinner("Running K-Means Clustering..."):
            try:
                ...
                cluster_results = {
                    ...
                    "covered_clusters": n_clusters
                    - len([c for c in cluster_dist.values() if c["test_count"] == 0]),
                    "uncovered_clusters": len(
                        [c for c in cluster_dist.values() if c["test_count"] == 0]
                    ),
                    "ood_pct": ood_pct,
                ...
                st.success("Cluster Coverage Check Complete!")
    ...
    if "eval_cluster_results" in st.session_state:
        ...
        if cr["coverage_pct"] >= 95:
            st.success(
                "✅ **Excellent Coverage**: Test data covers nearly all training input space clusters."
            )
        elif cr["coverage_pct"] >= 80:
            st.warning(
                "⚠️ **Good Coverage**: Most clusters are covered, but some regions of the input space are not represented in testing."
            )
        else:
            st.error(
                "🚨 **Poor Coverage**: Test data does not adequately cover the training input space. Model may be applied to unfamiliar data patterns."
            )

        if cr["ood_pct"] > 10:
            st.warning(
                f"⚠️ **OOD Alert**: {cr['ood_pct']:.1f}% of test samples are far from any training cluster center (potential out-of-distribution data)."
            )

        # OOD Samples Viewer & Download
        if cr["ood_count"] > 0 and "eval_ood_samples" in st.session_state:
            with st.expander(
                f"📋 View & Download OOD Samples ({cr['ood_count']} samples)", expanded=False
            ):
                ood_df = st.session_state["eval_ood_samples"]

                st.write("**Out-of-Distribution Samples**")
                st.caption(
                    "These test samples are far from any training cluster center, suggesting they may represent data patterns not seen during training."
                )

                # Show dataframe (sort by first column if _ood_distance not available)
                if "_ood_distance" in ood_df.columns:
                    display_df = ood_df.sort_values("_ood_distance", ascending=False).reset_index(
                        drop=True
                    )
                else:
        ...
        st.dataframe(
            pd.DataFrame(cr["cluster_summary"]).style.map(
                lambda v: (
                    "color: green"
                    if "Covered" in str(v)
                    else "color: red"
                    if "Uncovered" in str(v)
                    else ""
                ),
                subset=["Status"],
            )
        ...
        # Create bars
        bars_train = ax_clust.bar(
            x_pos - width / 2, train_pcts, width, label="Train", color="steelblue", alpha=0.8
        )
        bars_test = ax_clust.bar(
            x_pos + width / 2, test_pcts, width, label="Test", color="darkorange", alpha=0.8
        )
        ...
        ax_clust.set_ylim(0, max(*train_pcts, *test_pcts) * 1.2)
        ...
        st.caption(
            "💡 **How to read**: Gray = Training data. Colored squares = Test data. Each centroid shows **Train% / Test% (ratio)**."
        )
