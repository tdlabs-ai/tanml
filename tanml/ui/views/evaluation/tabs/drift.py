...
def render(context):
    ...
    from tanml.analysis.drift import calculate_ks, calculate_psi

    """Render the drift analysis tab."""

    st.markdown("### Feature Drift Analysis")
    st.caption(
        "Measures the shift in feature distributions between **Training** (Expected) and **Testing** (Actual) datasets."
    )
    ...
    if drift_results:
        ...
        **Thresholds:**
        ...
        if train_data.dtype in (bool, "bool"):
