...
def render_model_evaluation_page(run_dir):
    ...
    with st.container(border=True):
        ...
        target = c1.selectbox(
            "Target Column", all_cols, index=all_cols.index(curr_target), key="eval_target"
        )
        ...
        c1.markdown(
            f'<span class="task-badge">⚙️ Auto-detected Task: {task_type.title()}</span>',
            unsafe_allow_html=True,
        )
        ...
        features = c2.multiselect(
            "Feature Selection", possible_feats, default=default_feats, key="eval_features_ms"
        )
        ...
        # Model Form
        library, algo, hp, task_type = render_model_form(
            df_train[target], 42, target_name=f"eval_{target}"
        )
    ...
    for tab_obj, tab_def in zip(tab_objects, tabs, strict=False):
        with tab_obj:
            try:
