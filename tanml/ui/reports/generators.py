...
def _choose_report_template(task_type: str) -> Path:
    ...
    try:
        ...
        name = (
            "report_template_reg.docx" if task_type == "regression" else "report_template_cls.docx"
        )
...
def _generate_dev_report_docx(dev_data):
    ...
    if dev_data:
        ...
        p.add_run("This report summarizes the development of a ").bold = False
        p.add_run(f"{task.title()}").bold = True
        p.add_run(" model using ").bold = False
        ...
        if task == "classification":
            ...
            if score > 0.98:
                p2.add_run(
                    " (Note: Extremely high accuracy may indicate overfitting or data leakage.)"
                ).bold = True
        else:
            score = mets.get("r2", 0)
        ...
        if cv_met:
            ...
            cv_s = cv_met.get(
                ...
                cv_met.get(
                    ...
                    cv_met.get(
                        "R2",
                        cv_met.get(
                            "test_accuracy", cv_met.get("test_r2", cv_met.get("mean_test_score", 0))
                        ),
                    ),
                ),
            )
            ...
            if diff > 0.05:
                _add_md_paragraph(
                    doc,
                    f"⚠️ **Note**: Validation score ({score:.2f}) differs from Cross-Validation mean ({cv_s:.2f}) by >5%.",
                    style="List Bullet",
                )
            else:
                _add_md_paragraph(
                    doc,
                    "✅ **Stability**: Validation score aligns with Cross-Validation results.",
                    style="List Bullet",
                )
    else:
...
def _generate_eval_report_docx(buf):
    ...
    if drift:
        n_crit = sum(
            1
            for d in drift
            if "Critical" in str(d.get("PSI Status", ""))
            or "Critical" in str(d.get("KS Status", ""))
        )
        if n_crit > 0:
            _add_md_paragraph(
                doc, f"⚠️ **Summary**: {n_crit} features show Critical Drift.", style="List Bullet"
            )
        else:
            _add_md_paragraph(
                doc, "✅ **Summary**: No critical data drift detected.", style="List Bullet"
            )
    ...
    if stress and stress[0]:
        ...
        if abs(drop) > 0.1:
            _add_md_paragraph(
                doc,
                f"⚠️ **Summary**: Performance drops significantly ({abs(drop):.2%}) under noise.",
                style="List Bullet",
            )
        else:
            _add_md_paragraph(
                doc,
                "✅ **Summary**: Model is robust to noise (Stress Test passed).",
                style="List Bullet",
            )
    ...
    if (tr_s - te_s) > 0.15:
        warnings.append(
            f"Potential Overfitting: Train score ({tr_s:.2f}) is much higher than Test score ({te_s:.2f})."
        )
    ...
    if cluster_data:
        ...
        if ood_pct > 10:
            warnings.append(
                f"Significant Out-of-Distribution data detected ({ood_pct:.1f}% samples)."
            )
        if cov_pct < 80:
    ...
    doc.add_paragraph(
        f"This report covers {task_type.title()} performance, stability, and explainability checks."
    )
    ...
    if ev:
        ...
        if narrative:
            ...
            if (tr_s - te_s) > 0.15:
                doc.add_paragraph(
                    f"⚠️ **Attention**: Specific check indicates potential overfitting (Train {tr_s:.2f} vs Test {te_s:.2f}).",
                    style="List Bullet",
                )
        ...
        for k in sorted(all_keys):
            if k in ["confusion_matrix", "curves", "threshold_info"]:
                continue
            r = table.add_row().cells
            r[0].text = k
        ...
        if ev_imgs:
            ...
            for name in ev_imgs.keys():
                if name.startswith("train_"):
                    unique_bases.add(name.replace("train_", ""))
                elif name.startswith("test_"):
                    unique_bases.add(name.replace("test_", ""))
                else:
                    unique_bases.add(name)
            ...
            for base in sorted_bases:
                ...
                if train_key in ev_imgs and test_key in ev_imgs:
                    ...
                    r[0].add_paragraph().add_run().add_picture(
                        io.BytesIO(ev_imgs[train_key]), width=Inches(3.0)
                    )

                    # Test Right
                    p2 = r[1].add_paragraph("Test")
                    p2.alignment = 1  # Center
                    r[1].add_paragraph().add_run().add_picture(
                        io.BytesIO(ev_imgs[test_key]), width=Inches(3.0)
                    )
    ...
    if drift:
        ...
        if n_crit > 0:
            ...
            _add_md_paragraph(
                doc, "✅ **Summary**: No critical drift detected.", style="List Bullet"
            )
    ...
    if drift_data:
        ...
        for d in drift_data:
            ...
            r[1].text = f"{d['PSI']:.4f}"
            r[2].text = (
                str(d.get("PSI Status", d.get("Status", "")))
                .replace("🟢 ", "")
                .replace("🟠 ", "")
                .replace("🔴 ", "")
                .replace("🟡 ", "")
    ...
    if stress and stress[0]:
        ...
        if abs(drop) > 0.1:
            _add_md_paragraph(
                doc,
                f"⚠️ **Summary**: Performance drops significantly ({abs(drop):.2%}) under stress.",
                style="List Bullet",
            )
        else:
            _add_md_paragraph(
                doc,
                "✅ **Summary**: Model performance remains stable under stress.",
                style="List Bullet",
            )
    ...
    if stress_data:
        ...
        hdr = table.rows[0].cells
        for i, k in enumerate(stress_data[0].keys()):
            hdr[i].text = str(k)
        for row_dat in stress_data:
            r = table.add_row().cells
            for i, v in enumerate(row_dat.values()):
                r[i].text = f"{v:.4f}" if isinstance(v, float) else str(v)
    else:
        doc.add_paragraph("Stress test not run.")
    ...
    if cluster_data:
        ...
        if coverage_pct >= 95:
            _add_md_paragraph(
                doc,
                f"✅ **Excellent Coverage**: Test data covers {coverage_pct:.1f}% of the {n_clusters} training input space clusters.",
            )
        elif coverage_pct >= 80:
            _add_md_paragraph(
                doc,
                f"⚠️ **Good Coverage**: Test data covers {coverage_pct:.1f}% of clusters ({uncovered} uncovered).",
            )
        else:
            _add_md_paragraph(
                doc,
                f"🚨 **Poor Coverage**: Only {coverage_pct:.1f}% of training clusters are covered by test data.",
            )

        if ood_pct > 10:
            _add_md_paragraph(
                doc,
                f"⚠️ **OOD Alert**: {ood_pct:.1f}% of test samples appear to be out-of-distribution.",
            )
    ...
    if bench_data:
        ...
        bench_data.get("task_type", "classification")

        if baselines:
            doc.add_paragraph(
                f"Comparison of your model against {len(baselines)} baseline model(s) on test data."
            )
    ...
    if expl and expl.get("status") == "ok":
        ...
        if "top_features" in expl:
            ...
            for row in tf[:5]:
                r = table.add_row().cells
                k_feat = "feature" if "feature" in row else next(iter(row.keys()))
...
def _generate_ranking_report_docx(metrics_df, corr_df, method, target, task_type, X=None, y=None):
    ...
    if X is not None and y is not None:
        ...
        doc.add_paragraph(
            f"Distribution overlay for top {len(top_5)} features relative to target '{target}'."
        )
        ...
        for _idx, row in top_5.iterrows():
            ...
            try:
                ...
                if task_type == "classification":
                    # KDE Plot with Hue
                    sns.kdeplot(
                        data=X, x=feat, hue=y, fill=True, common_norm=False, palette="tab10", ax=ax
                    )
    ...
    _add_md_paragraph(
        doc,
        f"**Power Score**: {GLOSSARY.get('Power Score', 'Relative importance of a feature.')}",
        style="List Bullet",
    )
